...
class AnomalyDetectionService:
    ...
    def _load_environment_vars(self) -> None:
        """Загрузка обязательных переменных окружения."""
        self.vm_url = os.getenv('VICTORIAMETRICS_URL')
        if not self.vm_url:
            raise ValueError("Требуется переменная окружения VICTORIAMETRICS_URL")
            
        self.db_conn_string = os.getenv('DB_CONN_STRING')
        if not self.db_conn_string:
            raise ValueError("Требуется переменная окружения DB_CONN_STRING")
    ...
    def _init_service_components(self) -> None:
        ...
        try:
            ...
            conn_parts = self.db_conn_string.split('://')[1].split('@')
        ...
        self.anomaly_params = {
            ...
            'global': system_anomaly_config.get('min_confirmations', {}).get('global_anomaly', 1),
            'percentile_threshold': system_anomaly_config.get('percentile_threshold', 0.95)
        }
    ...
    async def _notify_anomaly(self, anomaly_data: Dict) -> None:
        ...
        if not self.notifier_url:
            self.logger.debug("URL сервиса уведомлений не задан, пропускаем отправку")
            return
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status != 200:
                        self.logger.error(f"Ошибка отправки уведомления: HTTP {resp.status}")
                    else:
                        self.logger.info(f"Уведомление об аномалии {anomaly_data['id']} отправлено")
        except Exception as e:
    ...
    def start(self) -> None:
        """Запуск сервиса."""
        try:
            self.logger.info("Запуск сервиса обнаружения аномалий")
            
            # Запуск воркеров
            num_workers = int(os.getenv('WORKER_THREADS', DEFAULT_WORKER_THREADS))
            self._start_workers(num_workers)
            
            self.logger.info(f"Сервис запущен с {num_workers} воркерами")
            ...
            self.logger.error(f"Ошибка запуска сервиса: {str(e)}", exc_info=True)
            raise
        finally:
            self.stop()
    ...
    def _start_workers(self, num_workers: int) -> None:
        ...
        for i in range(num_workers):
            ...
            self.logger.info(f"Запущен воркер {worker.name}")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                try:
                    ...
                    self.logger.error(f"Ошибка обработки задачи: {str(e)}", exc_info=True)
                finally:
                    self.task_queue.task_done()
            except queue.Empty:
                continue
            except Exception as e:
                self.logger.error(f"Ошибка воркера: {str(e)}", exc_info=True)
                time.sleep(5)
    ...
    def _process_task(self, task: Dict) -> None:
        ...
        try:
            ...
            if model_id not in self.active_models:
                self.logger.warning(f"Модель {model_id} не активна, пропускаем")
                return
            ...
            if not metric_data:
                self.logger.warning(f"Не удалось получить данные для модели {model_id} (метрика {metric_id})")
                return

            self.logger.info(f"Получено {len(metric_data)} точек для модели {model_id}")

            # 5. Загрузка модели (с кэшированием)
            model = self._load_model(model_id)
            if not model:
                self.logger.error(f"Не удалось загрузить модель {model_id}")
                return
            ...
            if predictions.size == 0:
                self.logger.warning(f"Не сгенерировано предсказаний для модели {model_id}")
                return
    ...
    def _process_and_save_results(self, actual: List[float], predicted: np.ndarray,
        ...
        try:
            ...
            if existing_points_count == 0:
                self.logger.info(f"Начальное окно: сохранено {saved_count} точек для модели {model_id}")
            else:
                self.logger.info(f"Сохранено {saved_count} предсказаний ({anomaly_count} аномалий) для модели {model_id}")
            
        except Exception as e:
            self.logger.error(f"Ошибка сохранения результатов для модели {model_id}: {str(e)}")
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    def _check_system_anomalies(self, cursor: Any, model_id: int, metric_id: int) -> None:
        ...
        try:
            ...
            """, (metric_id,))
            if cursor.fetchone():
                self.logger.debug(f"Активная аномалия для метрики {metric_id} уже существует")
                return
            ...
            try:
                ...
                self.logger.debug(f"Аномалия для метрики {metric_id} уже зарегистрирована")
                conn.rollback()
                return
                
        except Exception as e:
            self.logger.error(f"Ошибка проверки системных аномалий: {str(e)}")
            raise
    ...
    def _close_anomalies(self, cursor: Any, metric_id: int) -> None:
        try:
            ...
            self.logger.error(f"Ошибка при закрытии аномалий: {str(e)}")
    ...
    def _get_metric_data(self, metric_id: int, model_data: Dict) -> Optional[List[float]]:
        ...
        try:
            ...
            self.logger.error(f"Ошибка получения данных метрики {metric_id}: {str(e)}")
            return None
    ...
    def _load_model(self, model_id: int) -> Optional[Any]:
        ...
        try:
            ...
            self.logger.error(f"Ошибка загрузки модели {model_id}: {str(e)}")
    ...
    def _make_predictions(self, data: List[float], model: Any, 
        ...
        if not data or len(data) < 2:
            self.logger.warning("Недостаточно данных для предсказания")
            return np.zeros((1, 1))
    ...
    def _parse_time_window(self, window_str: str) -> timedelta:
        ...
        try:
            ...
            self.logger.warning(f"Неверный формат временного окна: '{window_str}'. Используется значение по умолчанию 3d.")
    ...
    def _get_anomaly_threshold(self, model_id: int, window_str: str = '3d') -> float:
        ...
        try:
            ...
            self.logger.error(f"Ошибка расчета порога для модели {model_id}: {str(e)}")
            return 0.0
    ...
    def _get_last_prediction_time(self, model_id: int, metric_id: int) -> Optional[datetime]:
        ...
        try:
            ...
            self.logger.error(f"Ошибка получения времени последнего предсказания: {str(e)}")
            return None
    ...
    def _check_model_version_active(self, model_info_id: int, version: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Ошибка проверки версии модели: {str(e)}")
            return False
            ...
            with self._get_db_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                    ...
                    if model := cursor.fetchone():
                        ...
                        self.logger.info(f"Добавлена активная модель {model['model_name']} (ID: {model_id})")
        except Exception as e:
            self.logger.error(f"Ошибка добавления активной модели: {str(e)}")
    ...
    def _remove_active_model(self, model_id: int) -> None:
        ...
        if model_id in self.active_models:
            ...
            self.logger.info(f"Удалена модель {model_name} (ID: {model_id})")
    ...
    def stop(self) -> None:
        """Остановка сервиса."""
        self.stop_event.set()
        self.logger.info("Сервис обнаружения аномалий остановлен")
...
if __name__ == "__main__":
    ...
    try:
        ...
        service.logger.info("Получен сигнал прерывания, остановка сервиса")
    except Exception as e:
        service.logger.error(f"Критическая ошибка: {str(e)}", exc_info=True)
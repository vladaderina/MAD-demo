...
class AnomalyNotifier:
    ...
    def _validate_config(self, config: Dict) -> None:
        ...
        if not config['mad_notifier'].get('bot_token'):
            raise ValueError("Необходимо указать TELEGRAM_BOT_TOKEN в переменных окружения")
        
        if not config['mad_notifier'].get('chat_id'):
    ...
    async def _send_telegram_message(self, text: str) -> bool:
        ...
        try:
            bot_token = self.config['mad_notifier'].get('bot_token')
            chat_id = self.config['mad_notifier'].get('chat_id')
